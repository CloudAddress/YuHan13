...
public:
    Player(float x, float y) : xPos(x), yPos(0.4), xVelocity(0.0f), yVelocity(0.0f), onGround(false), jumpStartTime(0.0), isJumping(false) {}
    ...
    void render() const {
        ...
        glBegin(GL_QUADS);
        glVertex2f(xPos - PLAYER_WIDTH / 1.6, yPos - PLAYER_HEIGHT / 1.2);
        glVertex2f(xPos + PLAYER_WIDTH / 1.6, yPos - PLAYER_HEIGHT / 1.2);
        glVertex2f(xPos + PLAYER_WIDTH / 1.6, yPos + PLAYER_HEIGHT / 1.2);
        glVertex2f(xPos - PLAYER_WIDTH / 1.6, yPos + PLAYER_HEIGHT / 1.2);
        glEnd();
...
public:
    Obstacle(float x, float y) : xPos(x), yPos(y), xVelocity(-0.9f), width(PLAYER_WIDTH), height(PLAYER_HEIGHT) {}
    ...
    void render() const {
        ...
        glBegin(GL_QUADS);
        glVertex2f(xPos - width / 2, (yPos * -0.09) - height * 3.85);
        glVertex2f(xPos + width / 2, (yPos * -0.09) - height * 3.85);
        glVertex2f(xPos + width / 2, (yPos * -0.09) + height * 3.85);
        glVertex2f(xPos - width / 2, (yPos * -0.09) + height * 3.85);
        glEnd();
...
int main() {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        glBegin(GL_QUADS);
        glVertex2f(-1.0f, GROUND_LEVEL - 0.083f);
        glVertex2f(1.0f, GROUND_LEVEL - 0.083f);
